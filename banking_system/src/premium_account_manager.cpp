...
namespace bank {
    std::unique_ptr<IAccount> PremiumAccountManager::createAccount(std::unique_ptr<Person> const& person, std::unique_ptr<IFunds> funds) {
        return std::make_unique<PremiumAccount>(person->m_id, std::move(funds));
    }
